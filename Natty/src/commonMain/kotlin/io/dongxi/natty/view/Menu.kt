...
class Menu(
    ...
    private var selectedItem by observable(items.first()) { _, selected ->
        ...
        animation = animate {
            ...
            items.filter { it != selected && !it.atDefaults }.forEach { deselected ->
                deselected.moveProgress to 0f using (tweenFloat(linear, itemMoveUpDuration)) {
                    deselected.moveProgress = it
                }
                deselected.selectionProgress to 0f using (tweenFloat(
                    linear,
                    itemFillDuration
                )) { deselected.selectionProgress = it }
            }

            // Indicator moves to selected item
            (indicatorCenter.x to selected.centerX using (tweenDouble(
                easeInOutCubic,
                slideDuration
            )) { indicatorCenter = Point(it, height) }).onCompleted {
                // Selected item moves down
                (selected.moveProgress to 1f using (tweenFloat(linear, itemMoveDownDuration)) {
                    selected.moveProgress = it
                }).also { secondaryAnimations += it }
            }

            // Indicator primes as it travels to selected item
            (indicatorHeight to minIndicatorHeight using (tweenDouble(linear, primeDuration)) {
                indicatorHeight = it
            }).onCompleted {
                ...
                (indicatorHeight to maxIndicatorHeight using (tweenDouble(linear, fireDuration)) {
                    indicatorHeight = it
                }).onCompleted {
                    // Indicator height returns to normal
                    (indicatorHeight to defaultIndicatorHeight using (tweenDouble(
                        linear,
                        recoilDuration
                    )) { indicatorHeight = it }).also { secondaryAnimations += it }

                    // Droplet moves up to item
                    (dropletYAboveIndicator to dropletMaxY using (tweenDouble(
                        linear,
                        dropletTravelDuration
                    )) { dropletYAboveIndicator = it }).onCompleted {
                        // Droplet is instantly hidden
                        dropletYAboveIndicator = 0.0

                        // Selected item moves up
                        (selected.moveProgress to 0f using (tweenFloat(
                            linear,
                            itemMoveUpDuration
                        )) { selected.moveProgress = it }).also { secondaryAnimations += it }

                        // Selected item animates droplet within it
                        (selected.selectionProgress to 1f using (tweenFloat(
                            linear,
                            itemFillDuration
                        )) { selected.selectionProgress = it }).also { secondaryAnimations += it }
                    }.also { secondaryAnimations += it }
                }.also { secondaryAnimations += it }
            }.also { secondaryAnimations += it }
        }
    ...
    override fun render(canvas: Canvas) {
        ...
        items.forEach { item ->
            ...
            canvas.transform(
                Identity.translate(Point(item.x, item.y + itemDipOffset * item.moveProgress))
                    ...
                    1f -> path(item.selected, fill = foreGround) // fully selected
                        ...
                        if (item.selectionProgress > 0f) {
                            ...
                            val dropletCircle = Circle(
                                ...
                                radius = dropLetRadius + (max(
                                    item.width,
                                    item.height
                                ) - dropLetRadius) * item.selectionProgress
                            )
    ...
    private fun updateIndicatorPath() {
        ...
        indicatorPath =
            path(Point(x = -indicatorHalfWidth)).cubicTo(Point(y = -indicatorHeight), controlPoint1, controlPoint2)
                .cubicTo(Point(x = indicatorHalfWidth), controlPoint3, controlPoint4).close()
    }

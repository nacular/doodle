...
open class BasicItemGenerator<T>(private val focusManager         : FocusManager?,
    ...
    override fun invoke(list: List<T, *>, row: T, index: Int, current: View?): View = when (current) {
        ...
        else          -> ListRow(list, row, index, list.itemVisualizer ?: passThrough(ToStringItemVisualizer(TextItemVisualizer(textMetrics))), selectionColor, selectionBlurredColor).apply {

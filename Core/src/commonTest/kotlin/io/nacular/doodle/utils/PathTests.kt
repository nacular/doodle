...
class PathTests {
    ...
    @Test @JsName("overlappingRootWorks")
    fun `overlapping root works`() {
    ...
    @Test @JsName("nonOverlappingStemWorks")
    fun `non-overlapping stem works`() {
        data class Inputs<T>(val first: List<T>, val second: List<T>, val expect: List<T>)
    ...
    @Test @JsName("rootIsAncestorToAll")
    fun `root is always ancestor`() {
        val paths = listOf(
            listOf(0, 0, 2, 5),
            listOf(0)
        )
